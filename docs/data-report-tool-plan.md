...
| 쟁점 | 결정 | 근거 |
|---|---|---|
| **ddof** (§14-Q5) | 표본 `stddev_samp`(n-1) | polars `std()` 기본 ddof=1. 행 단위 엔진과 동일 |
| **NULL 정책** (§14-Q6) | 있는 것만, 유효값 2개 미만이면 NULL | `group_by.std()`가 NULL을 무시. 행 단위와 동일 |
| **scale 적용 순서** (§14-Q7) | 스케일 후 수식 (기존 확정 그대로) | §6.0 처리 순서 ① SCALE → ② ABSOLUTE → ③ ADDP |

- `Std` 인자는 **순수 `{ALIAS}` 참조만 그룹 계산 대상**. `Std({A}/2, {B})`처럼
  순수 참조가 아니면 행 단위 폴백으로 계산한다 (거부가 아니라 폴백, 로그에 남음)
- 인자들의 `unit`이 서로 다르면 검증 단계에서 경고
- `SPREAD_N` 형제 item 자동 생성은 구현하지 않았다(⑤ 범위 밖) — std 값만 만든다
